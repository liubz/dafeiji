...
this.move=function(){let that=this;this.timer=setInterval(function(){let y=that.ele.offsetTop+that.speed;if(y>gameEngine.ele.offsetHeight){clearInterval(that.timer);gameEngine.ele.removeChild(that.ele);delete gameEngine.allEnemy[that.id];}
...
this.boom=function(){clearInterval(this.timer);let that=this;let i=0;let dieTimer=setInterval(function(){if(i>=that.dieImgs.length){clearInterval(dieTimer);gameEngine.ele.removeChild(that.ele);delete gameEngine.allEnemy[that.id];}
